...
**How to Identify if the state is duplicate?**\
**Ans:** If the `setter` function of a `useState` hook is only called inside `useEffect(s)` and not used elsewhere in the component (like returned JSX, or any other function).
